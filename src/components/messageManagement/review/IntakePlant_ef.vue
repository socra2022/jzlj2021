<template>
  <div style="height: 100%">
    <div id="title">
      <span class="span11">消纳场编号：{{ intakeid }}</span>
      <span class="span21">工程名称：{{ intakename }}</span>
...
<script>
// 导入请求数据的方法
import { selectOneIntakeData, selectByIdAndSOrI } from "../../../assets/axios/index";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getData() {
      // 请求点击的产生源基本信息
      selectOneIntakeData(this.id).then((res) => {
        // console.log(res.data.respBody);
        this.intakeData = JSON.parse(JSON.stringify(res.data.respBody));
        this.intakeid = this.intakeData[0].intakePlantId;
        this.intakename = this.intakeData[0].intakePlantName;

        // 请求点击的产生源的经纬度坐标信息
        selectByIdAndSOrI(0, this.id).then((res) => {
          // console.log(res);
