<template>
  <div style="height: 100%">
    <div id="title">
      <span class="span11">资源场编号：{{ resourceid }}</span>
      <span class="span21">工程名称：{{ resourcename }}</span>
...
<script>
// 导入请求数据的方法
import { selectOneResourceData, selectByIdAndSOrI } from "../../../assets/axios/index";
export default {
  data() {
    return {
  ...
  methods: {
    // 获取是点击的那一个工地
    getData() {
      selectOneResourceData(this.id).then((res) => {
        this.resourceData = JSON.parse(JSON.stringify(res.data.respBody));
        this.resourceid = this.resourceData[0].resourcePlantId;
        this.resourcename = this.resourceData[0].resourcePlantName;

        // 请求点击的消纳场的经纬度坐标信息
        selectByIdAndSOrI(2, this.id).then((res) => {
