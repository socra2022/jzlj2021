<template>
  <div>
    ...
    <div id="select">
      <span>页面名称：</span>
      <input type="text" class="input-style" placeholder="请输入页面名称" v-model="Source"/>
    ...
    >
      <el-form ref="addFormRef" :rules="addFormRules" :model="addForm" :inline="true">
        <el-form-item label="页面名称" prop="source">
          <el-input style="width:400px" v-model.number="addForm.source"></el-input>
        </el-form-item>
        <el-form-item label="页面描述" prop="sourcedesc">
          <el-input style="width:400px" v-model="addForm.sourcedesc"></el-input>
        </el-form-item>
        <el-form-item label="页面分组" prop="bygroup">
          <el-input style="width:400px" v-model="addForm.bygroup"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      >
        <el-form-item label="页面名称" prop="source">
          <el-input style="width:400px" v-model.number="updateForm.source"></el-input>
        </el-form-item>
        <el-form-item label="页面描述" prop="sourcedesc">
          <el-input style="width:400px" v-model="updateForm.sourcedesc"></el-input>
        </el-form-item>
        <el-form-item label="页面分组" prop="bygroup">
          <el-input style="width:400px" v-model="updateForm.bygroup"></el-input>
        </el-form-item>
...
export default {
  data() {
    return {
      ...
      addFormRules: {
        ...
        bygroup: [{ required: true, message: "不能为空!", trigger: "blur" }],
      },
  ...
  watch: {
    Source: function () {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (this.tableData[i].source.indexOf(input) != -1) {
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      //   size: this.PageSize,
      // };
      getPageSourceByPage(true, this.currentPage, this.PageSize).then(
        res => {
          //console.log(res);
 ...
 //请求所有数据
    async getData() {
      ...
      await getPageSource()
        ...
        .then({});
      await getPageSourceByPage(true, this.currentPage, this.PageSize).then(
        res => {
          console.log(res);
          this.allData = JSON.parse(JSON.stringify(res.data.respBody.records));
          this.tableData = JSON.parse(JSON.stringify(this.allData));
...
      }
      ...
      let data = {
        ...
        sourcedesc: this.addForm.sourcedesc,
        bygroup: this.addForm.bygroup,
      };
      createPageSource(data)
        .then(() => {
          this.$message({
            message: "添加成功！",
            type: "success",
...
      }
      ...
      let data = {
        id: this.currentId,
        source: this.updateForm.source,
        sourcedesc: this.updateForm.sourcedesc,
        bygroup: this.updateForm.bygroup,
      };
      console.log(data);
      //更新数据
      updateSP(data).then(() => {
    ...
    deleteData() {
      ...
      this.$confirm("确定要删除吗？", "提示", {
        ...
        deleteMPageSource(listData).then(() => {
          this.$message({
            message: "删除成功！",
            type: "success",
            offset: 300,
