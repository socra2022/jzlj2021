  ...
  <div id="sourceMonitor">
    <div id="leftContent" v-show="isShow">
      ...
      <table class="table">
        <tr>
          <th>编号</th>
          <td>{{ tableData.sourceId }}</td>
        </tr>
        <tr>
          <th>工地</th>
          <td>{{ tableData.sourceAddress }}</td>
        </tr>
        <tr>
          <th>排放状态</th>
          <td>{{ tableData.sourceCompany }}</td>
        </tr>
        <tr>
          <th>上报时间</th>
        ...
        <tr>
          <th>属性</th>
          <td>{{ tableData.sourceAttrbute }}</td>
        </tr>
        <tr>
          <th>垃圾类型</th>
          <td>{{ tableData.sourceType }}</td>
        </tr>
        <tr>
          <th>成分分析</th>
          <td>{{ tableData.wasteComponent }}</td>
        </tr>
        <tr>
          <th>建议</th>
          <td>{{ tableData.suggestion }}</td>
        </tr>
        <tr>
          <th>备注</th>
...
<script>
import { selectSourceData } from "../../assets/axios/index";
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          ...
          for (var i = 0; i < this.data_info.length; i++) {
            ...
            var marker = new BMap.Marker(
              new BMap.Point(
                this.data_info[i]["sourceLong"],
                this.data_info[i]["sourceLat"]
              ),
              { icon: myIcon }
            );
            markers.push(marker);
    ...
    //三维地图飞到目的地的方法
    flyToDestination() {
      let lon = this.tableData.sourceLong;
      let lat = this.tableData.sourceLat;
